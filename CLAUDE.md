...
- **Server components by default, small enumerated set of client islands** -- pages/layouts stay RSC; `"use client"` is limited to this list: the live score poller, the search command (⌘K / mobile dock search), the nav pills active-state, the season/franchise pickers, the player table (filter/sort), the draft countdown, the kickoff countdown, the smack composer, the commish claim-code reveal, the player-profile modal shell, the trades page deep-link scroll-focus island, the mobile nav scroll-chrome wrapper, the nav crest island, the smack composer slot, the nav live-pill island, the matchup-detail live score island, the book tab pill, the book board island, the book tracking island, and the book rail island. The board, tracking, and rail islands are on the list for the same reason the smack composer slot is: `/book` and `/` are both ISR-cached HTML served to the whole league, so the member's own slip (board), their own "YOU" tag plus their own not-yet-kicked-off picks (tracking), and their own live slip record (the hub rail card) all resolve client-side (session via `use-session-member.ts`, picks via `/api/book/picks`); the board island's picks are also re-enforced by the server action, while the tracking and rail islands are read-only. Streak Watch has no session dependency and stays a plain server component. The last four before those exist because the page HTML is ISR-cached: the first two resolve the session client-side so the server tree stays cacheable (sharing one `/api/session` fetch via `components/use-session-member.ts`), and the last two keep live scores and the live-game count honest inside cached HTML. Everything else ships zero client JS.
